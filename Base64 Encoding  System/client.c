...
/**
 ...
 - bitsLen = length of returned int array
 ...
 - Example :
		...
		len = 4
 **/
int *characters_to_binary(char *msg, int *len, int *bitsLen){
	int n = strlen(msg);
	int msgSize = n*8;
	if(msgSize%6)
		msgSize += (6 -msgSize%6);
	*len = msgSize/6;
	...
	if(n%3==2)
		(*len)+=1;
	*bitsLen = msgSize;
	int *bits = (int *)malloc(msgSize*sizeof(int));
	int idx = 0,i,j;
	for(i = 0;i < n;i++){
		int p  = (int)msg[i];
		for(j=0;j<8;j++){
...
void encode(char*data) {
	...
	int *bits = characters_to_binary(msg, &encoded_len, &bits_len);
